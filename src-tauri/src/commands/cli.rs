...
#[tauri::command]
pub(crate) async fn enable_cli_integration(state: State<'_, DesktopState>) -> Result<CliStatus, String> {
    let endpoint = get_active_proxy_endpoint(&state).await;
    let proxy_running = is_proxy_running(&state).await;
    if !proxy_running {
        return Err("请先启动 AGY BYOK 本地代理，再启用 CLI 接入".to_string());
    }
    let integration_root = state.ide_integration_root.clone();
    tauri::async_runtime::spawn_blocking(move || {
...
#[tauri::command]
pub(crate) async fn disable_cli_integration(state: State<'_, DesktopState>) -> Result<CliStatus, String> {
    let endpoint = get_active_proxy_endpoint(&state).await;
    let proxy_running = is_proxy_running(&state).await;
    let integration_root = state.ide_integration_root.clone();
    tauri::async_runtime::spawn_blocking(move || {
