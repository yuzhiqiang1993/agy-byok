...
- **系统体检与全链路健康诊断**：新增一键体检功能，深度探测本地代理、配置完整性、模型连通性与 Antigravity 宿主（IDE/App/CLI）状态，支持精准一键自愈与引导配置
- **自定义与官方模型压缩执行策略**：每个模型现在可以独立配置压缩策略，移除了全局配置钳制，UI 更直观
- **不兼容配置自动备份**：升级时若配置格式不兼容，自动备份旧配置文件，避免数据丢失

### 🔧 优化与重构

- **HTTP 连接池 LRU 淘汰**：优化代理连接池管理，减少内存占用和连接泄漏
- **前后端模型常量规范化**：内聚模型家族抽象，加固回退边界，提升路由精确匹配
- **请求解析优化与预检模块拆分**：连接测试解耦，提升代码可维护性
- **策略弹窗、官方卡片与目录投影拆分**：前端组件进一步解耦，减少单文件复杂度

### 📝 文档

- 将默认 README 切换为中文，英文版移至 README.en.md，更贴近国内开发者
...
- **图像生成与视频模型全面支持 (Image & Video Generation Support)**：
  - 支持将主流生图模型（如 FLUX、Midjourney、DALL-E、Recraft、Kolors、SDXL、Hunyuan-Image、Wanx 等）接入为自定义模型。
  - 为 OpenAI 兼容协议补齐 `/v1/images/generations` 转换与原生图片生成请求透传。
  - 支持生图模型类型切换、连接测试请求报文/响应报文实时透传与调试弹窗。
  - 自动识别官方生图模型请求并支持路由重定向至已启用的自定义生图模型。

- **Antigravity 新版模型聚合与档位适配 (Model Aggregation & Reasoning Levels)**：
  - 深度适配 Antigravity 最新版模型合并交互与推理档位（Low / Medium / High / Max / Auto / Adaptive 等）子菜单。
  - 支持自定义模型聚合母条目独立占位符与智能 Fallback 路由解析，未指定具体档位时自动优选默认档位。
  - 完善 Anthropic Adaptive Thinking 与多协议推理参数映射。
  - 将宿主 IDE/App 中的自定义模型标签文案统一优化为 BYOK 标识，并在标签位直观展示上游供应商名称。

- **自定义模型全生命周期管理 (Custom Model Editing)**：
  - 新增自定义模型就地编辑功能，支持在已接入模型卡片上直接修改协议、端点、模型映射及媒体能力声明。
  - 移除模型保存时的冗余二次确认拦截，优化交互流畅度。

### Improvements & UI Refinements / 体验优化

- **高级设置与表单组件规范统一**：统一高级设置折叠卡片高度规范与输入框规范，优化布局一致性与间距。
- **自定义模型 Slug 优化**：规范化自定义模型标识符生成逻辑，推理档位命名下划线转连字符，避免命名歧义。
- **配置自愈与安全清理**：应用启动时自动检测并清理历史不兼容或过期的配置字段，防止脏数据影响代理运行。

### Bug Fixes / 问题修复

- **宿主进程与环境恢复治理**：
  - 修复 IDE 模式恢复时本地代理设置遗留残留的问题。
  - 修复 App 模式恢复时残留进程唤醒与环境变量继承异常。
- **代理并发与资源竞争修复**：
  - 修复代理服务停止时的超时等待与内部互斥锁竞争问题。
  - 修复模型目录刷新时部分推理档位显示不正确的问题。
  - 移除多余重复的输出预留 Token 配置选项。
- **构建与 CI 检查**：
  - 统一 Rust 代码格式化规范并消除 Clippy 静态检查告警。
